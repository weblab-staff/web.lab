      ...
      <ul className="grid w-full grid-cols-2 items-center justify-center gap-4 sm:grid-cols-3 lg:grid-cols-4">
        {section.map((member, i) => (
          <li
            ...
            } flex h-full w-full flex-col gap-y-2 rounded-2xl border border-neutral-700 bg-transparent p-4 backdrop-blur backdrop-brightness-75 transition-transform ease-in-out [&>img]:grayscale [&>img]:hover:grayscale-0`}
            key={i}
            style={{ maxWidth: "600px" }}
          >
            <img
              className="relative aspect-square h-auto w-full overflow-hidden rounded-md border border-neutral-700 object-cover object-center transition duration-300 ease-in-out"
              src={
                currentTeam
